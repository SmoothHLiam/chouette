...
The app is a single page that draws itself with JavaScript, which used to mean
a crawler got `<main id="app"></main>` and nothing else — no text to rank, and
a stranger landing on a role dropdown with no idea what the site was.

* `index.html` now ships a real intro in the body. The router replaces it the
  moment the app boots, so it is what a visitor on a slow connection sees, what
  a crawler reads, and — with scripting off — what stays, minus the "Loading…"
  line, which would be a lie.
* The title and description carry **Chouette Learning** and English terms,
  because the interface is French but the students searching for it are not.
* `robots.txt`, `sitemap.xml`, a canonical URL, Open Graph and Twitter cards,
  and JSON-LD marking it a free `EducationalApplication`.
* `icons/og.png` is the card a shared link turns into, generated from the same
  owl by `tools/make-icons.js`.

The part no file can do: the site has to be submitted to
[Google Search Console](https://search.google.com/search-console) — verify the
domain, submit the sitemap, request indexing. Without that you are waiting to
be stumbled upon.

Worth knowing before you measure success by it: **"Chouette Learning" is not a
free name.** `chouette-learning.fr` is an established French training platform
using it, and `chouetteonapprend.org`, `chouett.com` and `chouetteofficiel.com`
are all nearby. Ranking for the exact phrase means outranking them. The domain
itself, and `chouettelearning` as one word, are winnable straight away.
...
icons/                  app icons and the share card, from tools/make-icons.js
robots.txt              crawl rules; keeps crawlers off /api/ and /__/
